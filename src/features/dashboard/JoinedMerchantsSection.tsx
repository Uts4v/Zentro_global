...
import { Loader2, ChevronRight, Gift, ShoppingBag, Coffee } from "lucide-react";
...
function MerchantCard({ merchant }: { merchant: JoinedMerchant }) {
  return (
    <div className="glass flex items-center gap-4 rounded-3xl p-4">
      ...
      >
        {!merchant.logo_url && <Coffee className="h-6 w-6 text-muted-foreground" />}
      </div>
  ...
  return (
    <section className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-rose-200 bg-rose-50 dark:bg-rose-950/30 px-4 py-3 text-sm text-rose-700">
          {error}
        </div>
      ) : merchants.length === 0 ? (
        <div className="glass rounded-3xl py-12 text-center">
          <div className="flex justify-center"><Coffee className="h-8 w-8 text-muted-foreground" /></div>
